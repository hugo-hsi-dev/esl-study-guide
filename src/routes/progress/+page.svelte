...
<main class="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 sm:py-12">
	<header class="space-y-3">
		...
		<p class="max-w-2xl text-zinc-700">
			See what changed, what your recent responses show, and when there is enough new evidence to
			reassess.
		</p>
	</header>
		...
		<div class="flex flex-wrap items-center justify-between gap-4">
			...
			{#if data.reassessment.recommended}
				<a
					...
					href="/assessment">Start reassessment</a
				>
			{:else}
				<p class="font-medium text-zinc-700">{data.reassessment.remaining} remaining</p>
			{/if}
		</div>
							...
							{#each session.results as result (result.sequence)}
								<div class="border-l-2 border-teal-600 pl-3 text-sm">
									...
									{#if result.feedback?.kind === 'objective'}
										<p class="text-zinc-700">{result.feedback.message}</p>
									{:else if result.feedback}
										<p class="text-zinc-700">
											{result.feedback.strength}
											{result.feedback.nextTip}
										</p>
									{/if}
								</div>
							{/each}
					...
					<thead
						><tr class="border-b border-zinc-200"
							><th class="py-3 pr-4">Signal</th><th class="py-3 pr-4">Area</th><th class="py-3 pr-4"
								>Evidence</th
							><th class="py-3">Recent accuracy</th></tr
						></thead
					>
					<tbody>
